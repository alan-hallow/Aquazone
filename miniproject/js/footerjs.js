// Get the logoofthewebsite element and all its child images
const logo = document.getElementById("logoofthewebsite");
const images = logo.querySelectorAll("img");

// Check if the logoofthewebsite is currently active
const getActive = () => document.body.dataset.active === "true";

// Set the active status of the logoofthewebsite
...
// Shift the logoofthewebsite based on mouse position
...
// Reset the logoofthewebsite's state
const resetLogo = () => {
  setActiveTo(false);
...
// Shift the logoofthewebsite on mouse move
window.onmousemove = e => shiftLogo(e, images);

// Reset the logoofthewebsite when the mouse leaves the window
document.body.onmouseleave = () => {
  if (!getActive()) resetLogo();
};

// Activate the logoofthewebsite on mouse down and shift based on mouse position
window.onmousedown = e => {
  setActiveTo(true);
  shiftLogo(e, images);
};

// Reset the logoofthewebsite on mouse up
window.onmouseup = e => resetLogo();

// Reset the logoofthewebsite's state initially
resetLogo();
